...
for t in range(T):
    m.addConstr(price[t] - c[3] - shadow[3, t] / df.cap[t] <= 0, "FOC1%d" % (t))
    m.addConstr(quantity[3, t] <= M * u1[3, t], "FOC2%d" % (t))
    m.addConstr(price[t] - c[3] - shadow[3, t] / df.cap[t] >= -M * (1 - u1[3, t]), "FOC3%d" % (t))
